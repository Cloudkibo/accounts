...
*/
const LogicLayer = require('./custom_field.logiclayer')
const MongoInterface = require('./interface_mongo')
const logger = require('../../../components/logger')
const CUSTOMFIELD = '/api/v1/kiboengage/custom_field/custom_field.datalayer.js'

const util = require('util')

exports.findAllCustomFieldObjects = () => {
  return MongoInterface.find()
}

exports.createOneCustomFieldObject = (body) => {
...
exports.updateCustomField = (body) => {
  if (body.purpose) {
...
exports.findCustomFieldsUsingQuery = (body) => {
  if (body.purpose) {
    // If purpose found, then proceed
    if (body.purpose === 'aggregate') {
      let aggregateQuery = LogicLayer.prepareMongoAggregateQuery(body)
      // If not validated
      logger.serverLog(CUSTOMFIELD, `Inside Aggregate: ${util.inspect(aggregateQuery)}`)
...
exports.deleteCustomField = (body) => {
  if (body.purpose) {
    let query = body.match
    // If purpose found, then proceed
