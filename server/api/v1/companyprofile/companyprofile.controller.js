      ...
      async.parallelLimit([
        function (cb) {
          PlanUsageModel.findOne({planId: planObject._id}).exec()
            .then(planUsage => cb(null, planUsage))
            .catch(err => {
              const message = err || 'Error in Find plan Usage' 
        ...
        function (cb) {
          CompanyUsageModel.findOne({companyId: req.user.companyId}).exec()
            .then(companyUsage => cb(null, companyUsage))
            .catch(err => { cb(err)
              const message = err || 'Error in Find companyUsage Usage' 
              logger.serverLog(message, `${TAG}: exports.updatePlan`, req.body, {companyId: req.user.companyId, user: req.user}, 'error')
              sendErrorResponse(res, 500, err)
            })
        },
        ...
        if (err) {
          ...
          if (companyUsage['facebook_pages'] > planUsage['facebook_pages']) {
            PagesModel.update({companyId: req.user.companyId}, {connected: false}, {multi: true}).exec()
              ...
              .catch(err => {
                logger.serverLog(err, `${TAG}: exports.updatePlan`, req.body, {companyId: req.user.companyId, user: req.user}, 'error')
                sendErrorResponse(res, 500, err)
              })
          } else {
            sendSuccessResponse(res, 200, '', 'Plan updated successfully!')
